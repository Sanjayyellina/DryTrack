...
const nav = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Bin Monitor', href: '/bins', icon: (
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Labor & Shifts', href: '/labor', icon: (
  ...
  },
  {
    group: 'Security',
    items: [
      { label: 'Verify Receipt', href: '/verify', icon: (
...
export default function Sidebar({ companyName }: { companyName: string }) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-60 bg-[#0F1923] flex flex-col z-40 border-r border-white/5">
      {/* Logo */}
      <div className="flex items-center gap-3 px-5 h-16 border-b border-white/5 flex-shrink-0">
        ...
        <div className="min-w-0">
          <div className="text-white font-bold text-sm leading-tight tracking-tight truncate">DryTrack</div>
          <div className="text-white/30 text-xs truncate">{companyName}</div>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3">
        {nav.map(section => (
          <div key={section.group} className="mb-5">
            <div className="text-white/25 text-[10px] font-semibold uppercase tracking-widest px-3 mb-1.5">
              {section.group}
            </div>
            {section.items.map(item => {
              ...
              return (
                ...
                >
                  <span className={active ? 'text-[#F5A623]' : 'text-white/30'}>{item.icon}</span>
                  {item.label}
                </Link>
              )
            })}
